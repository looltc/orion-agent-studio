...
export default function DashboardPage() {
  ...
  const handleConfigure = (agent: Agent) => {
    ...
    setForm({
      name: agent.name,
      role: agent.role,
      personalityName: agent.personality.name,
  ...
  const handleCreate = async () => {
    ...
    await agentStore.create({
      ...
      owner: "admin",
      personality: {
        name: form.personalityName.trim() || form.name.trim(),
        traits: form.traits
          .split(",")
          .map((t) => t.trim())
          .filter(Boolean),
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-surface-900 dark:text-white">Agent Team</h1>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-surface-500 text-sm">
            {search ? "No agents match your search." : agents.length === 0 ? "No agents yet. Create your first agent!" : "No agents in this status."}
          </p>
        </div>
      ) : (
          ...
          <div className="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl w-full max-w-md mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="px-5 py-4 border-b border-surface-200 dark:border-surface-700">
              <h3 className="font-semibold text-surface-900 dark:text-white">
                {editingAgent ? "Configure Agent" : "New Agent"}
              </h3>
            </div>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label className="text-xs text-surface-500 mb-1 block">Personality Name</label>
                <input
                  type="text"
                  value={form.personalityName}
                  onChange={(e) => setForm({ ...form, personalityName: e.target.value })}
                  className="w-full bg-surface-50 dark:bg-surface-950 border border-surface-200 dark:border-surface-700 rounded-lg px-3 py-2 text-sm text-surface-900 dark:text-white outline-none focus:border-orion-500/50"
                  placeholder="Architect"
                />
              </div>
              <div>
                <label className="text-xs text-surface-500 mb-1 block">Traits (comma-separated)</label>
                <input
                  type="text"
                  value={form.traits}
              ...
              <div>
                <label className="text-xs text-surface-500 mb-2 block">Capabilities</label>
              ...
              {/* System Prompt */}
              <div>
                <label className="text-xs text-surface-500 mb-1 block">System Prompt</label>
                <textarea
              ...
              {/* Skills */}
              <div>
                <label className="text-xs text-surface-500 mb-2 block">Skills</label>
                <div className="flex flex-wrap gap-2">
              ...
              {/* LLM Provider */}
              <div>
                <label className="text-xs text-surface-500 mb-1 block">LLM Provider</label>
                {providersLoading ? (
